...
router.post('/dashboard/api/training/candidates/:id/approve', requireAuth, express.json(), async (req, res) => {
  try {
    const { id } = req.params;
    const { corrected_response } = req.body || {};
    await db.query(`UPDATE public.scrapbot_training_candidates SET status = 'approved' WHERE id = $1`, [id]);
    const { rows } = await db.query(`SELECT * FROM public.scrapbot_training_candidates WHERE id = $1`, [id]);
    if (rows[0]) {
      await db.query(
...
router.post('/dashboard/api/training/candidates/:id/reject', requireAuth, express.json(), async (req, res) => {
  try {
    const { id } = req.params;
    const { corrected_response } = req.body || {};
    await db.query(`UPDATE public.scrapbot_training_candidates SET status = 'rejected' WHERE id = $1`, [id]);
...
router.get('/dashboard/api/training/export', requireAuth, async (req, res) => {
  try {
    ...
    await db.query(`UPDATE public.scrapbot_training_gold SET exported_at = now() WHERE exported_at IS NULL`);
...
router.post('/dashboard/api/training/collect', requireAuth, express.json(), async (req, res) => {
  try {
    const hours = parseInt((req.body && req.body.hours) || '24');
    const { collectKickExchanges } = await import('../services/trainingCollector.js');
    const kick = await collectKickExchanges(hours);
    return res.json({ ok: true, collected: { kick, total: kick } });
  } catch (e) {
    return res.status(500).json({ ok: false, error: e.message });
  }
});
