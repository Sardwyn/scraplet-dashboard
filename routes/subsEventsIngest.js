// /routes/subsEventsIngest.js
//
// Receives sub counter events (POST) and inserts into public.events
// so /w/:token/stream can relay them to OBS widgets.
//
// Also provides a Scrapbot-only pull endpoint (GET), mirroring raffle,
// so Scrapbot can fetch recent subs.update events per owner.
...
/**
 * POST /dashboard/api/subs/events
 * Body:
 *  {
 *    user_id: number,
 *    source: string,
 *    channel_slug: string,
 *    kind: string,           // usually "subs.update"
 *    payload: any,
 *    actor_id?: string,
...
router.post("/events", express.json({ limit: "256kb" }), async (req, res) => {
  try {
    ...
    console.error("[subsEventsIngest] POST /events error", e);
    return res.status(500).json({ ok: false, error: "server_error" });
  }
});

/**
 * GET /dashboard/api/subs/pull?owner_user_id=4&since=2026-01-03T08:00:00.000Z&limit=50&kind=subs.update
 *
 * Scrapbot-only pull endpoint to fetch recent subs events for an owner.
...
router.get("/pull", async (req, res) => {
  try {
    ...
    const kind = String(req.query.kind || "subs.update").trim();
    ...
    console.error("[subsEventsIngest] GET /pull error", e);
    return res.status(500).json({ ok: false, error: "server_error" });
  }
});
