...
const primitivesPath = path.join(__dirname, '../config/primitives.json');
...
function compileComponentRecipe(compId, variantId, x, y, width, height, paletteId, structureId, anchorZone) {
  ...
  // Resolve active variant adjective (Layer 3)
  ...
  for (const prim of recipe.primitives) {
    ...
    // Resolve structural roles inside component
  ...
  // Create Parent Subtract Boolean Wrapper if subtract masks exist
...
export async function applySceneTemplateInternal(overlayId, guildId, { archetypeId, variantId, structureId, paletteId, sceneIntent }) {
  ...
  for (const bp of variant.elements) {
    ...
    if (compId && recipes.components[compId]) {
      const variantId = bp.variantId || null;
      const compiled = compileComponentRecipe(compId, variantId, finalX, finalY, w, h, pId, sId, bp.anchorZone);
      if (compiled.length > 0) {
        overlay.json.elements.push(...compiled);
        continue;
      }
    }
